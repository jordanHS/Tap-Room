...
class KegControl extends React.Component {
  ...
  handleSellingPint = (id) => {
    this.setState({
      masterKegList: this.state.masterKegList.map(keg => {
        if (keg.id === id) {
          return {
            name: keg.name, alcohol: keg.alcohol, brand: keg.brand, pintsLeft: keg.pintsLeft - 1, id: keg.id, price: keg.price
          }
        }
        else { return keg }
      })
  ...
  render() {
    ...
    if (this.state.editing) {
      ...
      currentlyVisibleState = <KegDetail keg={this.state.selectedKeg} sellPint={this.handleSellingPint} onClickingEdit={this.handleEditClick} onClickingDelete={this.handleDeletingKeg} />
